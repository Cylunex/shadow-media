...
1. 优先使用每个 MediaSource 在 PlaybackInfo 中通告的 `DirectStreamUrl`，完整保留 MediaWarp 按
   HTTPStrm/AlistStrm 模式生成的路径与参数，避免客户端合成路由导致二次探测或慢回退；
2. `DirectStreamUrl` 若是 Emby 内网绝对地址或标准媒体路由，则重建到当前配置的反代 origin；真正的
   外部存储地址只作为本次 PlaybackInfo 的临时候选，不持久化；
3. 服务端未返回地址或通告地址失败时，才构造带 `MediaSourceId`、`PlaySessionId` 和 `Static=true`
   的标准 `/Videos/{id}/stream.{container}`；最后使用各版本的 `TranscodingUrl`/Emby HLS；
...
当前先遍历同一次 PlaybackInfo 返回的多版本通告直链 → 标准静态流回退 → 转码候选；候选全部失败后自动重新请求一次
