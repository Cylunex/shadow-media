...
更新：2026-09-10。这是开发分支的实际状态，不是「所有长期规划已完成」或正式发布公告。

前期逻辑走查见 [逻辑走查记录](LOGIC_AUDIT_2026_09_04.md)；本轮研究、设计修订与代码边界见
[vNext 补充研究与实现](VNEXT_RESEARCH_2026_09_10.md)。
...
ABS 整本音轨加入时只请求一次展开元数据，不逐轨预解析临时音频地址。M4B/ID3 文件内章节现已通过 Media3 `Chapter` 接入列表、当前章、时间范围和点击定位；真实文件样本与服务端全书章节映射仍需独立验收/实现。
...
- Room v4 的书架、定位、标注、同步队列与迁移标记保留；v5 新增听书队列头/条目和进度会话表，采用增量迁移。
- 读书、漫画、听书位置绑定打开时的资源版本和消费会话；事务内拒绝旧版本、旧会话和乱序事件，重读/回退正常保存。
- 听书队列持续保存当前位置、速度、循环和实际随机顺序；旧 preferences 一次性接管；重新连接恢复暂停队列，显式播放才加载资源。
- 队列中的重复曲目有独立 entryId，可分别移动/移除；Emby 播放上报按该实例绑定，普通下载不创建播放会话。
- MediaButtonReceiver 与三参数 onPlaybackResumption 已接入；System UI 重启推荐卡和 Auto 所需的 MediaLibraryService 尚未实现。
- 音频 UI 元数据/队列/状态由事件更新；连续进度只在界面可见且播放中计时。
...
2026-09-10 新增队列恢复/条目编辑写入、进度会话隔离和重复队列实例的 Emby 上报回归；
迁移脚本同时校验 v3→v4、v4→v5 的列、索引、外键与旧表样本数据保留。完整验证结果见
[本轮验证记录](VNEXT_RESEARCH_2026_09_10.md#6-验证记录)。
其后的全功能复查、章节导航和验证状态见 [三轮逻辑走查](LOGIC_AUDIT_2026_09_10.md)。
...
- ABS 全书章节映射与章节结束睡眠、通用断点下载任务中心、连续 Webtoon 跨页长条、后台 TTS、完整 TV 阅读交互。
