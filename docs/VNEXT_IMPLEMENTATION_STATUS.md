...
- 工作区要求 APK 构建与部署由用户明确提出；当前仅运行主机测试和测试源编译，未构建/安装 APK，未推送。

## 已有验证证据

- v6→v7 主机 SQLite 迁移通过同等字段、索引、外键及历史样本校验。
- v5→v6 主机 SQLite 实际执行迁移 SQL，与 Room 导出 schema 的字段、索引、外键一致，保留全部旧表样本数据。
- `test lintDebug` 通过：165 个独立测试（Debug 138 + 纯 JVM 27），包括 Release 与 benchmark 单测变体共 317 次执行，0 失败/错误/跳过；Lint 0 错误，94 条模块报告警告（含重复）。
- 未构建/安装 APK，未运行设备或真实服务器验收，未推送。2026-09-11 只读查询 ADB：已连接设备为 0。
