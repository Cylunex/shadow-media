...
- R1：从所有历史版本开始的 Android MigrationTestHelper；旧视频/阅读/音频继续状态只读聚合；更完整的账号/profile 作用域演进。
- R3：内嵌与服务端歌词优先级、远程歌单写回与检查点，音质能力的独立评估。
- R4：目录快照刷新代际/完整性状态、用户状态与个人收录分离、资源任务、Media3 明确离线副本管理、全链路仅离线约束、进程资源预算。
...
- `test lintDebug` 通过：130 个独立测试（Debug 113 + 纯 JVM 17），包括 Release 变体共 243 次执行，0 失败/错误/跳过；Lint 0 错误，83 条模块报告警告（含重复）。
