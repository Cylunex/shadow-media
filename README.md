...
- 影视仓订阅管理：安全检查用户自带 TVBox JSON、M3U 和 TXT，展示站点/直播数量与隔离运行时需求；
...
当前影视仓页面是安全的订阅与能力检查层，还不是完整 CatVod 执行器。远程 HTTP API、直播频道浏览、
统一搜索和换源会逐步接入统一 Provider；需要 JAR、QuickJS 或 Python 的站点只会标记为“需要隔离
