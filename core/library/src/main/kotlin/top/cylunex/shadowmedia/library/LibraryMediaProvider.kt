...
class LibraryMediaProvider(private val library: LibraryRepository) : MediaProvider {
    ...
    override suspend fun home() = listOf(ProviderSection("shelf", "最近加入", library.assets.first().take(20).map { it.item() }))
    ...
    private suspend fun page(query: String, type: String?, token: String?, size: Int): UnifiedMediaPage {
        val rows = library.assets.first().filter { (type == null || contentKind(type) == contentKind(it.kind)) &&
            (it.title.contains(query, true) || it.author.contains(query, true)) }
        val offset = token?.toIntOrNull()?.coerceAtLeast(0) ?: 0
        val items = rows.drop(offset).take(size.coerceIn(1, 200)).map { it.item() }
        return UnifiedMediaPage(items, nextPageToken = (offset + items.size).takeIf { it < rows.size }?.toString(), totalCount = rows.size)
    }
